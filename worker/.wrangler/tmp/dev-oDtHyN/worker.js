...
__name(generateUserIdFromDomain, "generateUserIdFromDomain");
...
async function handleSpeechRequest(request) {
  ...
  var speed = body.speed !== void 0 ? body.speed : 1;
  var pitch = body.pitch !== void 0 ? body.pitch : 1;
  var style = body.style || "general";
  var role = body.role || "";
  var styleDegree = body.styleDegree !== void 0 ? body.styleDegree : 1;
  ...
  var opts = {
    remove_markdown: cleaning_options.remove_markdown !== void 0 ? cleaning_options.remove_markdown : true,
    remove_emoji: cleaning_options.remove_emoji !== void 0 ? cleaning_options.remove_emoji : true,
    remove_urls: cleaning_options.remove_urls !== void 0 ? cleaning_options.remove_urls : true,
    remove_line_breaks: cleaning_options.remove_line_breaks || false,
    remove_citation_numbers: cleaning_options.remove_citation_numbers !== void 0 ? cleaning_options.remove_citation_numbers : true,
...
__name(handleSpeechRequest, "handleSpeechRequest");
...
__name(getVoiceStream, "getVoiceStream");
async function getAudioChunk(text, voiceName, rate, pitch, style, role, styleDegree, outputFormat, request) {
  ...
  if (style && style !== "general") {
    var attr = styleDegree !== 1 ? ' styledegree="' + styleDegree + '"' : "";
  ...
  var ssml = '<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" version="1.0" xml:lang="zh-CN"><voice name="' + voiceName + '">' + content + "</voice></speak>";
...
async function sign(urlStr) {
  ...
  var date = (/* @__PURE__ */ new Date()).toUTCString().replace(/GMT/, "").trim() + " GMT";
...
function cleanText(text, opts) {
  ...
  if (opts.remove_markdown) {
    t = t.replace(/!\[.*?\]\(.*?\)/g, "").replace(/\[(.*?)\]\(.*?\)/g, "$1").replace(/(\*\*|__)(.*?)\1/g, "$2").replace(/(\*|_)(.*?)\1/g, "$2").replace(/`{1,3}(.*?)`{1,3}/g, "$1").replace(/#{1,6}\s/g, "");
  }
  if (opts.custom_keywords) {
    var kw = opts.custom_keywords.split(",").map(function(k) {
      return k.trim();
    }).filter(function(k) {
      return k;
    });
    if (kw.length) {
      var pattern = kw.map(function(k) {
        return k.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&");
      }).join("|");
...
__name(getTestPageHTML, "getTestPageHTML");
...
// .wrangler/tmp/bundle-nTLhiU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nTLhiU/middleware-loader.entry.ts
